...
var longFormQuestion2_no = $('#crs_longquestion2_0'); 
  ...
  longFormQuestion2_no.change(function() {showHideQuestions(); });
    ...
    //long Form Question 3 Boolean variables
var longFormQuestion3_yes = $('#crs_longquestion3_1');
var longFormQuestion3_no = $('#crs_longquestion3_0'); 
  //long Form Question 3 Logic
  longFormQuestion3_yes.change(function() {showHideQuestions(); });
  longFormQuestion3_no.change(function() {showHideQuestions(); });
...
var lq3 = $('#crs_longquestion3yes').closest('td');
...
//longForm question 3 logic
if($('#crs_longquestion3_1').is(':checked')) {
  lq3.show("fast");
  $("#crs_longquestion3yes_label").parent().addClass('required');
 } else {
  lq3.hide("fast");
  $("#crs_longquestion3yes_label").parent().addClass('required');
 }
...
/*******************Create new validator - Long Form Question 3yes Textbox****************************/
var lq3yesValidator = document.createElement('span');
lq3yesValidator.style.display = "none";
lq3yesValidator.id = "RequiredFieldValidatorcrs_longquestion3yes";
lq3yesValidator.controltovalidate = "crs_longquestion3yes";
lq3yesValidator.errormessage = "<a href='#crs_longquestion3yes_label'> <b><u>'Purpose(s).'</u></b>   Is required, since you answered 'Yes' to the question above it. </a>";
lq3yesValidator.validationGroup = ""; // Set this if you have set ValidationGroup on the form
lq3yesValidator.initialvalue = "";
lq3yesValidator.evaluationfunction = function () {
    if (!$("#crs_longquestion3_1").is(':checked')){
        $("#crs_longquestion3yes_label").parent().addClass('required');
    return true; 
    }
    var value = $("#crs_longquestion3yes").val();
    if (value == null || value == "") {
    return false;
    } else {
    return true;
...
// Wire-up the click event handler of the validation summary link
$("a[href='#crs_longquestion3yes_label']").on("click", function () { scrollToAndFocus('crs_longquestion3yes_label','crs_longquestion3yes'); });
//Radio Button on change handler
$("#crs_longquestion3_1").on("change", function () { lq3yesToggleRequired('#crs_longquestion3_1'); });
$("#crs_longquestion3_1_0").on("change", function () { lq3yesToggleRequired('#crs_longquestion3_1'); });		 
//lq3yesToggleRequired('#crs_cmnquestion3_1')
//Toggle Required
function lq3yesToggleRequired(labelname){
      if ($(labelname).is(':checked')){
          $("#crs_longquestion3yes_label").parent().addClass('required');;
      }else{
          $("#crs_longquestion3yes_label").parent().removeClass('required');
      }		
}
// Add the new validator to the page validators array:
Page_Validators.push(lq3yesValidator);
